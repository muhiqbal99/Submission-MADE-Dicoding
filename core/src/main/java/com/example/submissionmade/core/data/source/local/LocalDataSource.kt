...
class LocalDataSource @Inject constructor(private val itemsDao: ItemsDao) {
    ...
    fun getMovieItem(): Flow<List<ItemsEntity>> =
        itemsDao.getItems("movie")

    fun getTvShowItem(): Flow<List<ItemsEntity>> =
        itemsDao.getItems("tvshow")

    fun getFavoriteItemList(): Flow<List<ItemsEntity>> =
        itemsDao.getFavoriteList()
